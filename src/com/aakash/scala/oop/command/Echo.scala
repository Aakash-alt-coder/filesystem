...
import com.aakash.scala.oop.files.{Directory, File}
...
class Echo(args: Array[String]) extends Command {
  override def apply(state: State): State = {
    /*
     1. if no args, state
     2. if one args, print to console
     3. if multiple args {
        operator = next to last argument
      if >
         echo to the file (may create a file if not there)
       >>
         append to the file
       else
  ...
  def getRootAfterEcho(currentDirectory: Directory, path: List[String], contents: String, append: Boolean): Directory = {
    /*
     if path is empty , then fail (currentDirectory)
       ...
       find the next  directory to navigate to
  ...
  def doEcho(state: State, contents: String, filename: String, append: Boolean): State = {
    if(filename.contains(Directory.SEPARATOR))
      state.setMessage("Echo: filename should not contain separators")
    else {
